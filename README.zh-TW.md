...
[為什麼需要](#為什麼需要) · [安裝](#安裝) · [能問什麼](#能問什麼) · [Vault 報告](#vault-報告) · [技術架構](#技術架構) · [貢獻](#想貢獻)
...
2. **任一支援的 AI agent** — Claude Code、OpenClaw、Cursor、Cline、GitHub Copilot、Windsurf 等 30+ 種
...
| 「幫我產出知識圖譜報告」 | 串接 vault-stats、hubs、bridges、orphans，產出 4 模組結構化報告 |
...
**vault-report** skill（隨附安裝）能產出完整的知識圖譜分析報告。它串接多個圖查詢，產出 4 模組的結構化報告：

| 模組 | 涵蓋內容 |
|------|---------|
| **全景概覽** | 筆記總數、連結數、孤島率、連通性、資料夾分布 |
| **活躍度趨勢** | 月度新增筆記數、活躍高峰期 |
| **知識重心與結構** | 樞紐筆記 Top 10、各領域連結密度、跨領域連結、孤島分布 |
| **知識價值與風險** | 被引用最多的筆記、沒人引用的筆記、Bridge 關鍵節點（斷了就散的樞紐） |

### 實際對話長這樣

> **你：** 幫我產出知識圖譜報告
>
> **Claude：** *（依序跑 vault-stats、hubs、orphans-rich、bridges）*
>
> **Vault 知識圖譜報告**
>
> | 指標 | 數值 |
...
> | 最大分量涵蓋率 | 83.4% |
>
> 樞紐筆記第一名：**MOC 索引**（75 條連結）— 跨領域連結佔比：21.7%
>
> Bridge 風險：**神經網路**（degree 42）— 移除後心理學和資工區塊會斷開
...
| **vault-stats** | 全掃描 + BFS | O(V+E) | 一次性全 vault 統計：節點/邊數、孤島率、連通分量、各資料夾統計、跨資料夾連結、月度趨勢、未被引用筆記 |

### 參數化設計

模板中有兩類佔位符，Agent 執行前從 `vault-config.md` 讀取值並代入：

- **`{{EXCLUDED_FOLDERS}}`** — 排除資料夾的 JSON 陣列，8 個模板都有
...
| vault-stats componentSizes 最多 20 個、outOnlyNotes 最多 50 筆 | 避免輸出爆量 |
