...
> **Why**: `MAX_DETAIL = 50` — 超過 50 個鄰居時截斷輸出，避免 JSON 過大導致 LLM context 爆掉。50 是在「資訊完整」和「輸出可控」之間的平衡點。
...
> **Why**: `MAX_DISPLAY = 500` — 超過 500 個節點逐一列出會產生巨大 JSON，改為按資料夾計數既省 token 又保留結構資訊。
...
> **Why**: 遞迴版 Tarjan 在大型 vault（2000+ 節點）會觸發 V8 的 call stack limit。改用 iterative + 手動 stack 模擬解決。輸出限制 50 條橋接邊 + 30 個關鍵節點，足夠識別結構弱點而不灌爆 context。
